...
func main() {
	...
	if err = (&perfdatasourcegitlab.ReconcilePerfDataSourceGitLab{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("perf-data-source-gitlab"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "perf-data-source-gitlab")
		os.Exit(1)
	}

	if err = (&perfdatasourcejenkins.ReconcilePerfDataSourceJenkins{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("perf-data-source-jenkins"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "perf-data-source-jenkins")
		os.Exit(1)
	}

	if err = (&perfdatasourcesonar.ReconcilePerfDataSourceSonar{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("perf-data-source-sonar"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "perf-data-source-sonar")
		os.Exit(1)
	}

	if err = (&perfserver.ReconcilePerfServer{
		Client: mgr.GetClient(),
		Scheme: mgr.GetScheme(),
		Log:    ctrlLog.WithName("perf-server"),
	}).SetupWithManager(mgr); err != nil {
